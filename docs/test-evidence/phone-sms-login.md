...
脚本把这些值传给当前 API 进程，不输出值，也不写入用户或系统环境变量。
...
本轮不发送真实短信，也不启动模拟器。确认阿里云配置完整、两个号码已绑定且允许计费后，再执行：

```powershell
docker compose --env-file .env -f deploy/docker-compose.yml up -d
dotnet run --project src/MentalHealth.Api/MentalHealth.Api.csproj --urls http://127.0.0.1:5165
...
启动 API 前，必须通过本机环境或 Secret Manager 提供数据库、JWT 和 `PhoneLogin` 配置，并设置 `PhoneLogin:Aliyun:Enabled=true`。管理端使用管理手机号完成一次登录；Android 16 使用客户端手机号完成一次登录。记录中不得写入完整手机号、短信验证码、AccessKey、`ekey`、挑战令牌或 JWT。
