...
describe('phone login form', () => {
  it('separates the management identity from the phone login task', () => {
    ...
    expect(introduction.find('h1').text()).toBe('心续管理端')
    expect(introduction.text()).toContain('把每天的记录、一次咨询和后续回访接起来。')
    expect(introduction.text()).toContain('日常资料默认仅本人可见，系统不提供心理诊断。')
